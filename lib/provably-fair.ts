...
export async function generateSeed(length = 16): Promise<string> {
  let result = ""
...
export async function combineSeeds(serverSeed: string, clientSeed: string, nonce: number): Promise<string> {
  return await hashSeed(`${serverSeed}:${clientSeed}:${nonce}`)
}

// Generate a random number between 0 and 1
export async function generateRandomNumber(combinedSeed: string): Promise<number> {
...
export async function verifyServerSeed(revealedServerSeed: string, hashedServerSeed: string): Promise<boolean> {
  return (await hashSeed(revealedServerSeed)) === hashedServerSeed
}

// Generate a shuffled deck based on the server seed and client seed
export async function generateShuffledDeck(serverSeed: string, clientSeed: string): Promise<number[]> {
  ...
  for (let i = deck.length - 1; i > 0; i--) {
    ...
    const combinedSeed = await combineSeeds(serverSeed, clientSeed, i)
    const randomValue = await generateRandomNumber(combinedSeed)
